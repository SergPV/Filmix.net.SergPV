        ...
        try:
            try:
                ...
                try:
                    ...
                    raise
            trans_link = re.compile('data-translation-link="([^\"]+)"', re.S).findall(html)
            trans_name = re.compile('data-locked="no">([^\"]+)</span></li>', re.S).findall(html)
            j = 0
            for js_string in trans_link:
                js_string = self.decode_direct_media_url(js_string.decode('string_escape').decode('utf-8'))
                if(js_string.find('.txt') != -1):
                    ...
                    for season in movies['playlist']:
                        current_movie = {'folder_title' : season['comment']+' ('+trans_name[j]+')', 'movies': {}}
                    ...
                    current_movie = {'folder_title' : trans_name[j], 'movies': {}}
                    for q in avail_quality:
                        if(q == ''): continue
